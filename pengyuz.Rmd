...
```{r, echo=TRUE}
red_wine <- read.csv("http://archive.ics.uci.edu/ml/machine-learning-databases/wine-quality/winequality-red.csv", sep = ";")
red_wine <- red_wine[, c("free.sulfur.dioxide", "total.sulfur.dioxide")]
head(red_wine)
```
```{r}
# the following code is suppose to get the same data, but my rstudio cant load it.
# library(rattle)
# data(winequality)
# head(winequality$red)
```

## Compare two types of kernel
```{r}
# Set the seed for reproducibility
set.seed(52)

# Split the data into training and testing sets
train_indices <- sample(1:nrow(red_wine), 0.6 * nrow(red_wine))  # 70% of data for training
train_data <- red_wine[train_indices, ]
test_data <- red_wine[-train_indices, ]

# Perform kernel smoothing
log_train_predictor <- log(train_data$total.sulfur.dioxide)
normal_kernel_model <- ksmooth(log_train_predictor, train_data$free.sulfur.dioxide, kernel = "normal", bandwidth = 0.8)
box_kernel_model <- ksmooth(log_train_predictor, train_data$free.sulfur.dioxide, kernel = "box", bandwidth = 0.8)
```
...
plot_data <- data.frame(log_total_sulfur_dioxide = log(train_data$total.sulfur.dioxide),
                        free_sulfur_dioxide = train_data$free.sulfur.dioxide)

# Combine the x and y values of both kernel models into a single data frame
kernel_data <- data.frame(x = c(normal_kernel_model$x, box_kernel_model$x),
                          y = c(normal_kernel_model$y, box_kernel_model$y),
                          model = rep(c("Normal", "Box"), each = length(normal_kernel_model$x)))

ker<- rbind(data.frame(normal_kernel_model, model = "Normal"),
            data.frame(box_kernel_model, model = "Box"))

ggplot(data = plot_data, aes(x = log_total_sulfur_dioxide, y = free_sulfur_dioxide)) +
  geom_point(shape = 1) +
  geom_line(data = ker, aes(x = x, y = y, color = model)) +
  labs(title = "Kernel Smoothing Model Line on Training Data)",
       x = "Logged Total Sulfur Dioxide",
...
## Kernel model on test data and test the accuracy
```{r}
# plot the training kernel on the testing point
# Create the ggplot object
normal_kernel_df <- data.frame(x = normal_kernel_model$x, y = normal_kernel_model$y) # create a data frame for normal kernel model
box_kernel_df <- data.frame(x = box_kernel_model$x, y = box_kernel_model$y) # create a data frame for box kernel model
plot_data <- data.frame(log_total_sulfur_dioxide = log(test_data$total.sulfur.dioxide),
                        free_sulfur_dioxide = test_data$free.sulfur.dioxide)
ggplot() +
  geom_point(data = plot_data, aes(x = log_total_sulfur_dioxide, y = free_sulfur_dioxide), shape = 1) +  # plot points
  geom_line(data = ker, aes(x = x, y = y, color = model)) +
  labs(title = "Kernel Smoothing Model Line on Training Data)",
       x = "Logged Total Sulfur Dioxide",
       y = "Free Sulfur Dioxide",
...
# truncate the test data. When evaluating a model, it's essential to ensure that the data used for testing falls within the same range as the data used for training. If the test data contains values outside the range of the training data, the model might encounter situations it hasn't seen before, leading to unreliable predictions.
test_data_truncated <- test_data[log(test_data$total.sulfur.dioxide) >= min(log_train_predictor) & 
                                 log(test_data$total.sulfur.dioxide) <= max(log_train_predictor), ]
# compute RMSE for gaussian kernel
normal_kernel_model_prediction <- approx(normal_kernel_model$x, normal_kernel_model$y, log(test_data_truncated$total.sulfur.dioxide))
normal_kernerl_model_accuracy <- sqrt(mean((normal_kernel_model_prediction$y - test_data_truncated$free.sulfur.dioxide)^2))
cat("RMSE for normal kernerl model: ", normal_kernerl_model_accuracy, "\n")

# compute RMSE for box kernel
box_kernel_model_prediction <- approx(box_kernel_model$x, box_kernel_model$y, log(test_data_truncated$total.sulfur.dioxide))
box_kernerl_model_accuracy <- sqrt(mean((box_kernel_model_prediction$y - test_data_truncated$free.sulfur.dioxide)^2))
cat("RMSE for box kernerl model: ", box_kernerl_model_accuracy, "\n")
```
